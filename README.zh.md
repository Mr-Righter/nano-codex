...
- `Agent 中间件` 对一次完整的智能体循环执行 pre-process 与 post-process。适用于重写消息列表、调整运行级选项或注入影响整个循环的提醒信息。
- `Chat 中间件` 对循环内的每次 LLM 请求/响应执行 pre-process 与 post-process。适用于在内容到达模型之前进行转换，或在后续阶段消费之前重塑模型响应。
- `Function 中间件` 对每一次单独的工具调用执行 pre-process 与 post-process。可用于检查已验证的工具参数、附加元数据，或在工具结果返回至循环之前对其进行改写。

这些分层分别服务于智能体循环级、LLM 调用级以及工具调用级的上下文工程任务。
...
| 中间件 | 层级 | 用途 |
| --- | --- | --- |
| `user_message_reminder` | Agent | 通过在最新用户消息旁插入提醒，对当前智能体循环执行 pre-process。 |
| `logging_response` | Chat | 通过将聊天响应转换为助手事件，对 chat 响应执行 post-process。 |
| `move_tool_media_to_user_message` | Chat | 通过将工具返回的媒体内容改写为后续用户消息，对下一次模型输入执行 pre-process。 |
| `strip_reasoning` | Chat | 在请求发送到下游之前移除 reasoning items，对 chat 请求执行 pre-process。 |
| `tool_result_reminder` | Function | 通过追加后续提醒，对选定的工具结果执行 post-process。 |
| `logging_function_result` | Function | 通过发出结构化的工具生命周期事件，对工具执行同时进行 pre-process 与 post-process。 |
